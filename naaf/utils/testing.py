...
def assert_data_equal(data1, data2):
    ...
    for field in d1.keys():
        if isinstance(d1[field], (np.ndarray, da.Array)):
            ...
            assert set(d1[field].columns) == set(
                d2[field].columns
            ), f'dataframe fields "{field}" have different columns'
            for col in d1[field]:
                if isinstance(d1[field][col][0], Rotation):
                    ...
                    if not np.allclose(back_forth, 0):
                        raise AssertionError(
                            f'Rotation columns "{col}" of field "{field}" are not equivalent'
                        )
                else:
                    try:
                        if not np.allclose(d1[field][col], d2[field][col]):
                            raise AssertionError(
                                f'numeric array columns "{col}" of field "{field}" are not within error'
                            )
                    except TypeError:
                        if not np.all(d1[field][col] == d2[field][col]):
                            raise AssertionError(
                                f'array columns "{col}" of field "{field}" are not equal'
                            )
        else:
